...
## 開発経緯
マイコンなどのハードウェア上で暗号処理を行う際に漏洩するスクリーミングチャネル情報を取得するシステムは一般的には数十万円以上するオシロスコープとファンクションジェネレータを使用して実験を行う必要があったため、研究室外で手軽に実験を行えるように
macOSなどのPCとHackRFとBLEデバイスだけで実験を行えるシステムを開発しました。
